...
plot_matrix <- plot_ly(data, x = ~precipitation, y = ~temperature, z = ~casos, type = "scatter3d",
                       mode = "markers", marker = list(size = 3, color = ~casos)) %>%
              layout(title = "Scatter Plot Matrix",
                     scene = list(xaxis = list(title = "Total Precipitation"),
                                  yaxis = list(title = "Temperature"),
                                  zaxis = list(title = "Dengue Cases")))
